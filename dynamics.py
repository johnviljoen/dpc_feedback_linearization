...
@eqx.filter_jit
def f(qp, fp, x, u, d=[0.]*3, g=9.81, K_p_w=0.05, Ts=0.001): # L_p_w=0.05

    """
    parameters:
        qp = quad parameters
        fp = feedback linearization parameters
    ...
    v_r = u[3:6] - 2 * fp["Lambda"] @ tilde_p - fp["Lambda"] @ fp["Lambda"] @ x[17:20]
    dv_r = u[6:9] - 2 * fp["Lambda"] @ (x[7:10] - u[3:6]) - fp["Lambda"] @ fp["Lambda"] @ tilde_p
    s = u[3:6] - v_r
    hat_f_a = jnp.zeros(3) # Estimate interaction forces (assuming zero for single vehicle)
    f_d = qp["mB"] * dv_r - fp["K"] @ s - qp["mB"] * jnp.array([0, 0, -g]) - hat_f_a
    ...
    e_omega = x[10:13]
    tau_d = -fp["K_R"] @ e_R - fp["K_omega"] @ e_omega
